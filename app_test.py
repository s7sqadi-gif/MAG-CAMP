...
TXT={'ar':{'dash':'الرئيسية','workers':'العمال','rooms':'الغرف','requests':'الطلبات','approvals':'الاعتمادات','users':'المستخدمون','audit':'سجل النشاط','logout':'خروج','lang':'English','title':'نظام إدارة وتشغيل مساكن مجموعة المجال العربي','site':'سكن ولي العهد'},'en':{'dash':'Dashboard','workers':'Workers','rooms':'Rooms','requests':'Requests','approvals':'Approvals','users':'Users','audit':'Activity Log','logout':'Logout','lang':'العربية','title':'MAG Housing Operations Management System','site':'Wali Al Ahad Camp'}}
...
class App(BaseHTTPRequestHandler):
 ...
 def login(self,msg=''):
  self.html(f'<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="/static/style.css"></head><body class="login"><form class="loginbox" method="post"><img src="/static/mag_logo.png"><h2>نظام إدارة وتشغيل مساكن مجموعة المجال العربي</h2><p>MAG Housing Operations Management System</p>{"<div class=notice>"+e(msg)+"</div>" if msg else ""}<input name="username" placeholder="اسم المستخدم / Username" required><br><br><input type="password" name="password" placeholder="كلمة المرور / Password" required><br><br><select name="lang"><option value="ar">العربية</option><option value="en">English</option></select><br><br><button class="btn primary" style="width:100%">دخول / Login</button></form></body></html>')
 def login_post(self):
...
if __name__=='__main__':ThreadingHTTPServer(('127.0.0.1',8765),App).serve_forever()
