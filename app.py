...
APP_VERSION='9.0.0-RC1'
RELEASE_NAME='9.0-rc1-stable-production-recovery'
...
def normalize_ar_day(v):
 v=str(v or '').strip().replace('اﻷ','الأ')
 aliases={'اثنين':'الاثنين','الإثنين':'الاثنين','ثلاثاء':'الثلاثاء','الثلاثا':'الثلاثاء','اربعاء':'الأربعاء','الاربعاء':'الأربعاء','أربعاء':'الأربعاء','خميس':'الخميس','جمعة':'الجمعة','الجمعه':'الجمعة','سبت':'السبت','احد':'الأحد','الاحد':'الأحد','أحد':'الأحد'}
...
def bootstrap_postgres_from_sqlite():
    ...
    if migration_already_verified():
        # Never trust an old marker by itself. The stable 7.3 data must still
        # be present in PostgreSQL; otherwise the recovery is run again.
        verified, details = verify_all_tables()
        if verified:
            print(f'[MAG CAMP] Stable data already verified: {details}', flush=True)
            return
        print(f'[MAG CAMP] Recovery marker exists but data is incomplete: {details}', flush=True)
...
def ensure_schema():
 with closing(conn()) as c:
  ...
  -- Production indexes for the most frequently loaded dashboards and lists.
  CREATE INDEX IF NOT EXISTS idx_users_employee_active ON users(employee_no,active);
  CREATE INDEX IF NOT EXISTS idx_users_username_active ON users(username,active);
  CREATE INDEX IF NOT EXISTS idx_rooms_zone_room ON rooms(zone,room_no);
  CREATE INDEX IF NOT EXISTS idx_rooms_supervisor ON rooms(supervisor_id,zone);
  CREATE INDEX IF NOT EXISTS idx_workers_active_room ON workers(archived,room_no);
  CREATE INDEX IF NOT EXISTS idx_workers_employee ON workers(employee_no);
  CREATE INDEX IF NOT EXISTS idx_workers_iqama ON workers(iqama_no);
  CREATE INDEX IF NOT EXISTS idx_tickets_location_status ON maintenance_tickets(location_type,location_id,status);
  CREATE INDEX IF NOT EXISTS idx_housing_actions_worker_status ON housing_actions(worker_id,final_status);
  CREATE INDEX IF NOT EXISTS idx_absence_worker_status ON absence_reports(worker_id,status);
  ''')
  ...
  if existing:
   ...
   if not collision:
    c.execute("INSERT INTO users(employee_no,username,display_name,password_hash,role,preferred_lang,active,must_change_password) VALUES(?,?,?,?,?,?,?,?)",('admin','admin','مدير النظام الشامل',make_hash(admin_password),'super_admin','ar',1,0))
  c.commit()

# Production recovery: restore the verified 7.3 dataset when PostgreSQL is empty/partial.
if IS_POSTGRES:
 try:
  from production_recovery import recover_if_needed
  recover_if_needed(BUNDLED_SQLITE_SOURCE)
 except Exception as exc:
  print(f"[MAG CAMP 9.0] production recovery warning: {exc}", file=sys.stderr, flush=True)
bootstrap_postgres_from_sqlite()
ensure_schema()

# Account passwords are preserved from the stable 7.3 database.
# No automatic mass reset runs during application startup.
...
def assigned_zones(c,u):
 if u['role']!='housing_supervisor':
  return [str(r['zone_name']) for r in c.execute("SELECT zone_name FROM bathroom_complexes WHERE active=1 GROUP BY zone_name ORDER BY CAST(zone_name AS INTEGER),zone_name").fetchall()]
 return [str(r['zone']) for r in c.execute("SELECT zone FROM rooms WHERE supervisor_id=? AND zone IS NOT NULL GROUP BY zone ORDER BY CAST(zone AS INTEGER),zone",(u['id'],)).fetchall()]
...
</style></head><body><div class="top"><div class="brand"><img src="{{url_for('static',filename='mag_logo.png')}}" alt="MAG"><div><b>MAG CAMP</b><br><small>{{t.system_name}} — Enterprise 9.0 RC1</small></div></div>{% if u %}<div class="userline">{{u['display_name']}}<br><small>{{roles.get(u['role'],u['role'])}} | <a style="color:white" href="{{url_for('change_password')}}">{{t.change_password}}</a> | <a style="color:white" href="{{url_for('logout')}}">{{t.logout}}</a></small></div>{% endif %}</div>{% if u %}<nav class="mobile-nav"><a href="{{url_for('dashboard')}}">{{t.home}}</a><a href="{{url_for('notifications')}}">{{t.notifications}}</a>{% if not maintenance_only %}<a href="{{url_for('global_search')}}">{{t.search}}</a><a href="{{url_for('rooms')}}">{{t.rooms}}</a><a href="{{url_for('occupancy_management')}}">{{t.occupancy}}</a><a href="{{url_for('workers')}}">{{t.workers}}</a>{% if u.role=='housing_supervisor' %}<a href="{{url_for('inspections')}}">{{t.inspections}}</a>{% endif %}<a href="{{url_for('sector_dashboard')}}">القطاعات</a><a href="{{url_for('worker_change_requests')}}">إدارة الطلبات</a><a href="{{url_for('worker_exit_requests')}}">إنهاء حالة عامل</a><a href="{{url_for('absence_reports_list')}}">بلاغات عدم التواجد</a>{% if can_attendance %}<a href="{{url_for('attendance_batches')}}">حصر الغياب</a>{% endif %}{% if admin %}<a href="{{url_for('reports_center')}}">مركز التقارير</a>{% endif %}{% endif %}<a href="{{url_for('tickets')}}">{{t.maintenance}}</a></nav><div class="wrap"><aside class="desktop-nav"><a href="{{url_for('dashboard')}}">{{t.home}}</a><a href="{{url_for('notifications')}}">{{t.notifications}}</a>{% if not maintenance_only %}<a href="{{url_for('global_search')}}">{{t.search}}</a><a href="{{url_for('workers')}}">{{t.workers}}</a><a href="{{url_for('rooms')}}">{{t.rooms}}</a><a href="{{url_for('occupancy_management')}}">{{t.occupancy}}</a>{% if u.role=='housing_supervisor' %}<a href="{{url_for('inspections')}}">{{t.inspections}}</a>{% endif %}<a href="{{url_for('sector_dashboard')}}">القطاعات</a>{% endif %}{% if not maintenance_only %}<a href="{{url_for('worker_change_requests')}}">إدارة الطلبات</a><a href="{{url_for('worker_exit_requests')}}">إنهاء حالة عامل</a><a href="{{url_for('absence_reports_list')}}">بلاغات عدم التواجد</a>{% if can_attendance %}<a href="{{url_for('attendance_batches')}}">حصر الغياب</a>{% endif %}{% if admin %}<a href="{{url_for('reports_center')}}">مركز التقارير</a>{% endif %}{% endif %}<a href="{{url_for('tickets')}}">{{t.maintenance}}</a>{% if u.role in ('super_admin','maintenance_manager','maintenance_supervisor','housing_manager','services_manager') %}<a href="{{url_for('maintenance_dashboard')}}">{{t.maintenance_dashboard}}</a>{% endif %}{% if admin %}<a href="{{url_for('users')}}">{{t.users}}</a><a href="{{url_for('admin_monitor')}}">مراقبة النظام</a><a href="{{url_for('password_change_admin')}}">سجل كلمات المرور</a><a href="{{url_for('audit_logs')}}">{{t.audit}}</a><a href="{{url_for('backup_restore')}}">النسخ الاحتياطي</a>{% endif %}<a href="{{url_for('change_password')}}">{{t.change_password}}</a></aside><main>{{body|safe}}</main></div>{% else %}{{body|safe}}{% endif %}<script>
function addPhotoInput(containerId,inputName){
 const box=document.getElementById(containerId);
 if(!box)return;
 const row=document.createElement('div');
 row.style.marginTop='8px';
 const input=document.createElement('input');
 input.type='file';input.name=inputName;input.accept='image/*';input.setAttribute('capture','environment');
 const remove=document.createElement('button');
 remove.type='button';remove.className='btn danger';remove.style.marginInlineStart='6px';remove.textContent='حذف';
 remove.onclick=function(){row.remove();};
 row.appendChild(input);row.appendChild(remove);box.appendChild(row);
}
</script></body></html>'''
...
def choose_language():
 ...
 return render_template_string('''<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Select Language</title><style>body{font-family:Tahoma,Arial;background:#f4f6f8;margin:0}.box{max-width:520px;margin:12vh auto;background:#fff;padding:35px;border-radius:18px;box-shadow:0 5px 24px #cdd4d1;text-align:center}.logo{width:105px}.choices{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:25px}.choice{border:0;border-radius:12px;padding:18px;background:#123b32;color:white;font-size:18px;cursor:pointer}@media(max-width:540px){.box{margin:8vh 14px}.choices{grid-template-columns:1fr}}.weekly-rest-row{background:#ffe5e5!important;border-right:5px solid #b91c1c}.repeat-row:not(.weekly-rest-row){background:#fff8d6}.same-room-badge{background:#7f1d1d;color:#fff;border-radius:999px;padding:3px 8px;font-size:12px}.filter-panel{position:sticky;top:108px;z-index:10}.filter-panel select,.filter-panel input{min-width:140px}.hidden-row{display:none!important}@media(max-width:760px){.filter-panel{position:static}.filter-panel .grid{grid-template-columns:1fr 1fr}.attendance-table{font-size:12px}}
</style></head><body><div class="box"><img class="logo" src="{{url_for('static',filename='mag_logo.png')}}"><h1>MAG CAMP</h1><p>اختر اللغة | Select Language</p><form method="post" class="choices"><button class="choice" name="language" value="ar">العربية</button><button class="choice" name="language" value="en">English</button></form></div></body></html>''')
...
def workers():
 ...
 with closing(conn()) as c:
  rows=c.execute(sql,p).fetchall();zones=c.execute('SELECT zone FROM workers WHERE archived=0 AND zone IS NOT NULL GROUP BY zone ORDER BY CAST(zone AS INTEGER)').fetchall();total=c.execute('SELECT COUNT(*) FROM workers WHERE archived=0').fetchone()[0]
...
def room_detail(room_no):
 ...
 with closing(conn()) as c:
  ...
  if not r:abort(404)
  if request.method=='POST':
   if u['role'] not in ('super_admin','services_manager','housing_manager'):abort(403)
...
def inspections():
 ...
 with closing(conn()) as c:
  worker_names_agg="STRING_AGG(DISTINCT CASE WHEN w.id IS NOT NULL THEN w.employee_no||' - '||w.full_name END, '، ')" if IS_POSTGRES else "GROUP_CONCAT(DISTINCT CASE WHEN w.id IS NOT NULL THEN w.employee_no||' - '||w.full_name END)"
  room_rows=c.execute(f'''SELECT r.room_no,r.zone,r.capacity,COUNT(DISTINCT w.id) occupied,
   {worker_names_agg} worker_names,
...
def new_worker_change_request():
 ...
 err='';typ=request.form.get('action_type') or request.args.get('type') or ({'delete':'remove'}.get(request.form.get('change_type'),request.form.get('change_type'))) or 'add'
 if request.method=='POST':
...
def ticket_detail(tid):
 ...
 with closing(conn()) as c:
  ...
  # جميع مسؤولي الصيانة (مدير الصيانة ومشرف الصيانة) يملكون صلاحية فتح البلاغات
  # حتى بعد إسناد البلاغ لأحدهما؛ الإسناد يحدد المسؤول التنفيذي ولا يمنع الاطلاع أو المتابعة.
  if u['role'] not in ('super_admin','services_manager','housing_manager','maintenance_manager','maintenance_supervisor') and u['role'] not in ('housing_supervisor','housing_monitor','data_entry'):
   abort(403)
  if request.method=='POST':
 ...
 {% if maintenance_only and ticket.status=='in_progress' %}<form method="post" enctype="multipart/form-data"><textarea name="notes" placeholder="ملاحظات التنفيذ" required></textarea><div id="after-photo-inputs"><input type="file" name="after_photos" accept="image/*" capture="environment" required></div><button type="button" class="btn btn2" style="margin-top:8px" onclick="addPhotoInput('after-photo-inputs','after_photos')">+ إضافة صورة أخرى</button><small class="muted" style="display:block;margin:6px 0">يمكن رفع عدة صور بعد الإصلاح، صورة بعد صورة.</small><button class="btn" name="action" value="complete">تم التنفيذ وإرسال للمبلغ</button></form>{% endif %}
...
def available_beds():
 ...
 with closing(conn()) as c:
  ...
  sql+=' GROUP BY r.id,u.display_name HAVING COUNT(w.id)<COALESCE(r.capacity,0) ORDER BY CAST(r.zone AS INTEGER),CAST(r.room_no AS INTEGER)'
  rows=c.execute(sql,params).fetchall();zones=[x['zone'] for x in c.execute(f'''SELECT r.zone FROM rooms r WHERE {cl} AND COALESCE(r.usage_type,'residential')='residential' GROUP BY r.zone ORDER BY CAST(r.zone AS INTEGER)''',args).fetchall()]
...
def export_workers_excel():
 ...
 logo_path=os.path.join(ROOT,'static','mag_logo.png')
 if os.path.exists(logo_path):
  logo=XLImage(logo_path);logo.width=150;logo.height=55;ws.add_image(logo,'S1')
...
def export_workers_pdf():
 ...
 def draw_header(page_no):
  logo_path=os.path.join(ROOT,'static','mag_logo.png')
  if os.path.exists(logo_path):
   try:cvs.drawImage(logo_path,margin,h-42,width=70,height=28,preserveAspectRatio=True,mask='auto')
...
ABSENCE_REASONS=['راحة أسبوعية','سيتم إرساله إلى الحرم','مريض داخل السكن','تم نقله إلى المستشفى','غير متواجد في السكن','يوم راحة','إجازة سنوية','تم تغيير الوردية','رفض الذهاب إلى العمل','غياب بدون عذر','غير مقيم في السكن','منقول إلى مشروع آخر','خروج نهائي','أخرى']
...
def attendance_batches():
 ...
 if request.method=='POST':
  ...
  if not items:err='لم يتم العثور على أرقام وظيفية صالحة. '+'، '.join(errors)
   ...
   absence_date=saudi_today().isoformat();shift_label=request.form.get('shift_label','')
   with closing(conn()) as c:
    ...
    for item in items:
     ...
     today_name=AR_WEEKDAYS[saudi_today().weekday()]
     automatic_reason='راحة أسبوعية' if normalize_ar_day(rest_day)==today_name else ''
     automatic_note='صُنّف تلقائيًا: يوم رفع الملف هو يوم الراحة الأسبوعية المسجل.' if automatic_reason else ''
     c.execute('INSERT INTO attendance_entries(batch_id,employee_no,full_name,profession,shift,rest_day,is_resident,room_no,zone,source_files,previous_absences,reason,notes,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(bid,eno,full_name,profession,shift,rest_day,1 if resident else 0,resident['room_no'] if resident else '',str(resident['zone']) if resident else '','، '.join(item['file_names']),previous,automatic_reason,automatic_note,now()))
...
def attendance_batch_detail(bid):
 ...
 room_counts={r['room_no']:0 for r in rows if r['room_no']}
 for r in rows:
  if r['room_no']:room_counts[r['room_no']]=room_counts.get(r['room_no'],0)+1
 weekly_count=sum(1 for r in rows if r['reason']=='راحة أسبوعية')
 unresolved=sum(1 for r in rows if not (r['reason'] or '').strip())
 return page("""<h2 class='attendance-title'>حصر الغياب {{b.batch_no}}</h2><div class='cards'><div class='card'><div>إجمالي الوارد</div><div class='num'>{{rows|length}}</div></div><div class='card'><div>راحة أسبوعية</div><div class='num'>{{weekly_count}}</div></div><div class='card'><div>الغياب الفعلي</div><div class='num'>{{rows|length-weekly_count}}</div></div><div class='card'><div>المتبقي بدون سبب</div><div class='num'>{{unresolved}}</div></div></div>{% if missing %}<p class='err'>متبقي {{missing}} عامل بدون سبب غياب.</p>{% endif %}<div class='card'><p><b>التاريخ:</b> {{b.absence_date}} · <b>أعده:</b> {{b.creator}} · <b>الحالة:</b> {{'معتمد' if b.status=='completed' else 'مسودة'}}</p><p class='err'>الصف الأحمر يعني أن العامل ظهر في كشف الحرم رغم أن اليوم هو يوم راحته الأسبوعية.</p><a class='btn' href='{{url_for("attendance_export_excel",bid=b.id)}}'>Excel Dashboard</a> <a class='btn btn2' href='{{url_for("attendance_export_pdf",bid=b.id)}}'>PDF Dashboard</a>{% if admin and b.status=='completed' %}<form method='post' style='display:inline'><button class='btn danger' name='action' value='reopen'>إعادة فتح</button></form>{% endif %}</div><div class='card filter-panel'><h3>تصفية وترتيب القائمة</h3><div class='grid'><div class='field'><label>بحث</label><input id='attSearch' placeholder='الاسم أو الرقم الوظيفي'></div><div class='field'><label>الزون</label><select id='zoneFilter'><option value=''>الكل</option>{% for z in zones %}<option>{{z}}</option>{% endfor %}</select></div><div class='field'><label>الغرفة</label><select id='roomFilter'><option value=''>الكل</option>{% for r in rooms %}<option>{{r}}</option>{% endfor %}</select></div><div class='field'><label>الوردية</label><select id='shiftFilter'><option value=''>الكل</option>{% for x in shifts %}<option>{{x}}</option>{% endfor %}</select></div><div class='field'><label>الحالة</label><select id='statusFilter'><option value=''>الكل</option><option value='weekly'>راحة أسبوعية</option><option value='missing'>بدون سبب</option><option value='repeat'>غياب متكرر</option><option value='resident'>مقيم</option><option value='nonresident'>غير مقيم</option><option value='same_room'>أكثر من متغيب في الغرفة</option></select></div><div class='field'><label>&nbsp;</label><button type='button' class='btn btn2' onclick='resetAttendanceFilters()'>إظهار الجميع</button></div></div><p><b id='visibleCount'>{{rows|length}}</b> من أصل {{rows|length}} عامل ظاهر الآن.</p></div><div class='card'><h3>ملخص الأسباب</h3>{% for x in summary %}<span class='badge'>{{x.reason}}: {{x.count}}</span> {% endfor %}</div><form method='post'><div class='card'><div class='grid'><div class='field'><label>تطبيق سبب على المحددين</label><select name='bulk_reason'><option value=''>اختر السبب</option>{% for r in reasons %}<option>{{r}}</option>{% endfor %}</select></div><div class='field'><label>&nbsp;</label><button class='btn' name='action' value='save'>حفظ التعديلات</button></div></div></div><div class='tbl-wrap'><table class='tbl attendance-table'><tr><th>تحديد</th><th>الرقم</th><th>الاسم</th><th>الغرفة</th><th>الزون</th><th>الوردية</th><th>الراحة</th><th>السكن</th><th>الغياب السابق</th><th>مصدر الملف</th><th>السبب</th><th>ملاحظات</th></tr>{% for x in rows %}<tr class='attendance-row {% if x.reason=="راحة أسبوعية" %}weekly-rest-row{% elif x.previous_absences>=5 %}repeat-row{% endif %}' data-search='{{x.employee_no}} {{x.full_name}}' data-zone='{{x.zone or ""}}' data-room='{{x.room_no or ""}}' data-shift='{{x.shift or ""}}' data-weekly='{{1 if x.reason=="راحة أسبوعية" else 0}}' data-missing='{{1 if not x.reason else 0}}' data-repeat='{{1 if x.previous_absences>=5 else 0}}' data-resident='{{1 if x.is_resident else 0}}' data-roomcount='{{room_counts.get(x.room_no,0)}}'><td><input type='checkbox' name='selected' value='{{x.id}}'><input type='hidden' name='entry_id' value='{{x.id}}'></td><td>{{x.employee_no}}</td><td>{{x.full_name}}</td><td>{{x.room_no or '-'}} {% if x.room_no and room_counts.get(x.room_no,0)>1 %}<span class='same-room-badge'>{{room_counts.get(x.room_no)}} من نفس الغرفة</span>{% endif %}</td><td>{{x.zone or '-'}}</td><td>{{x.shift or '-'}}</td><td>{{x.rest_day or '-'}}</td><td>{{'مقيم' if x.is_resident else 'غير مقيم'}}</td><td><b>{{x.previous_absences}}</b>{% if x.previous_absences>=5 %}<br><span class='badge red'>غياب متكرر</span>{% endif %}</td><td>{{x.source_files}}</td><td><select name='reason_{{x.id}}' {% if b.status=='completed' %}disabled{% endif %}><option value=''>اختر</option>{% for r in reasons %}<option {% if x.reason==r %}selected{% endif %}>{{r}}</option>{% endfor %}</select>{% if x.reason=='راحة أسبوعية' %}<br><b class='err'>راحة أسبوعية تلقائيًا</b>{% endif %}</td><td><input name='notes_{{x.id}}' value='{{x.notes or ""}}' {% if b.status=='completed' %}disabled{% endif %}></td></tr>{% endfor %}</table></div>{% if b.status!='completed' %}<div class='card'><button class='btn' name='action' value='save'>حفظ</button> <button class='btn btn2' name='action' value='complete'>اعتماد الحصر النهائي</button></div>{% endif %}</form><script>
function applyAttendanceFilters(){const q=document.getElementById('attSearch').value.trim().toLowerCase(),z=document.getElementById('zoneFilter').value,r=document.getElementById('roomFilter').value,sh=document.getElementById('shiftFilter').value,st=document.getElementById('statusFilter').value;let n=0;document.querySelectorAll('.attendance-row').forEach(row=>{let ok=(!q||row.dataset.search.toLowerCase().includes(q))&&(!z||row.dataset.zone===z)&&(!r||row.dataset.room===r)&&(!sh||row.dataset.shift===sh);if(st==='weekly')ok=ok&&row.dataset.weekly==='1';if(st==='missing')ok=ok&&row.dataset.missing==='1';if(st==='repeat')ok=ok&&row.dataset.repeat==='1';if(st==='resident')ok=ok&&row.dataset.resident==='1';if(st==='nonresident')ok=ok&&row.dataset.resident==='0';if(st==='same_room')ok=ok&&Number(row.dataset.roomcount)>1;row.classList.toggle('hidden-row',!ok);if(ok)n++});document.getElementById('visibleCount').textContent=n}
['attSearch','zoneFilter','roomFilter','shiftFilter','statusFilter'].forEach(id=>document.getElementById(id).addEventListener(id==='attSearch'?'input':'change',applyAttendanceFilters));function resetAttendanceFilters(){['attSearch','zoneFilter','roomFilter','shiftFilter','statusFilter'].forEach(id=>document.getElementById(id).value='');applyAttendanceFilters()}
</script>""",'تفاصيل حصر الغياب',u,b=b,rows=rows,summary=summary,reasons=ABSENCE_REASONS,missing=missing,admin=is_admin(u),weekly_count=weekly_count,unresolved=unresolved,room_counts=room_counts,zones=sorted({str(r['zone']) for r in rows if r['zone']}),rooms=sorted({str(r['room_no']) for r in rows if r['room_no']}),shifts=sorted({str(r['shift']) for r in rows if r['shift']}))
...
def project_workers_import():
 ...
 msg='';err=''
 if request.method=='POST':
  f=request.files.get('project_file'); update_shift=bool(request.form.get('update_shift')); update_rest=bool(request.form.get('update_rest')); update_name=bool(request.form.get('update_name')); update_profession=bool(request.form.get('update_profession'))
  try:
   from openpyxl import load_workbook
   ...
   with closing(conn()) as c:
    for ws in wb.worksheets:
     ...
     for row in ws.iter_rows(min_row=hi+1,values_only=True):
      ...
      existing=c.execute('SELECT * FROM project_workers WHERE employee_no=?',(eno,)).fetchone()
      full_name=val(cn) if update_name or not existing else existing['full_name']; profession=val(cp) if update_profession or not existing else existing['profession']; shift=val(cs) if update_shift or not existing else existing['shift']; rest_day=val(cr) if update_rest or not existing else existing['rest_day']
      c.execute('INSERT INTO project_workers(employee_no,full_name,profession,shift,rest_day,source_sheet,source_location,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(employee_no) DO UPDATE SET full_name=excluded.full_name,profession=excluded.profession,shift=excluded.shift,rest_day=excluded.rest_day,source_sheet=excluded.source_sheet,source_location=excluded.source_location,updated_at=excluded.updated_at',(eno,full_name,profession,shift,rest_day,ws.title,val(cl),now()));count+=1
...
def attendance_history():
 ...
 with closing(conn()) as c:
  agg="STRING_AGG(DISTINCT e.reason, '، ')" if IS_POSTGRES else "GROUP_CONCAT(DISTINCT e.reason)"
  rows=c.execute(f'SELECT e.employee_no,MAX(e.full_name) full_name,MAX(e.room_no) room_no,MAX(e.shift) shift,COUNT(*) total,MAX(b.absence_date) last_date,{agg} reasons FROM attendance_entries e JOIN attendance_batches b ON b.id=e.batch_id WHERE {where} GROUP BY e.employee_no ORDER BY total DESC,last_date DESC LIMIT 1000',params).fetchall()
...
def attendance_export_excel(bid):
 ...
 dash.merge_cells('A1:H2');dash['A1']='MAG CAMP — لوحة حصر غياب العمالة'
 logo_path=os.path.join(ROOT,'static','mag_logo.png')
 if os.path.exists(logo_path):
  logo=XLImage(logo_path);logo.width=145;logo.height=52;dash.add_image(logo,'J1');dash['A1'].font=Font(size=18,bold=True,color='FFFFFF');dash['A1'].fill=PatternFill('solid',fgColor='17365D');dash['A1'].alignment=Alignment(horizontal='center',vertical='center')
...
def attendance_export_pdf(bid):
 ...
 logo_path=os.path.join(ROOT,'static','mag_logo.png')
 if os.path.exists(logo_path):
  try:story.append(RLImage(logo_path,width=110,height=42))
  except Exception:pass
...
def backup_restore():
 ...
 msg='';err=''
 if IS_POSTGRES:
  return page('''<div class="card"><h2>النسخ الاحتياطي</h2><p>قاعدة الإنتاج PostgreSQL على Supabase. يتم النسخ الاحتياطي من لوحة Supabase أو بواسطة ملف الترحيل.</p></div>''','النسخ الاحتياطي',u)
 if request.method=='POST':
...
def admin_monitor():
 ...
 db_size=0 if IS_POSTGRES else (os.path.getsize(DB) if os.path.exists(DB) else 0)
 ...
 with closing(conn()) as c:
  integrity='ok' if IS_POSTGRES else c.execute('PRAGMA integrity_check').fetchone()[0]
  stats={
...
def users():
 ...
 if not is_admin(u):abort(403)
 with closing(conn()) as c:
  agg="STRING_AGG(a.room_text, '، ')" if IS_POSTGRES else "GROUP_CONCAT(a.room_text,'، ')"
  rows=c.execute(f"SELECT u.*,{agg} room_text FROM users u LEFT JOIN assignments a ON a.user_id=u.id GROUP BY u.id ORDER BY u.active DESC,u.role,u.display_name").fetchall()
