...
</style></head><body><div class="top"><div class="brand"><img src="{{url_for('static',filename='mag_logo.png')}}" alt="MAG"><div><b>MAG CAMP</b><br><small>{{t.system_name}} — Enterprise 4 Final</small></div></div>{% if u %}<div class="userline">{{u['display_name']}}<br><small>{{roles.get(u['role'],u['role'])}} | <a style="color:white" href="{{url_for('logout')}}">{{t.logout}}</a></small></div>{% endif %}</div>{% if u %}<nav class="mobile-nav"><a href="{{url_for('dashboard')}}">{{t.home}}</a><a href="{{url_for('notifications')}}">{{t.notifications}}</a>{% if not maintenance_only %}<a href="{{url_for('global_search')}}">{{t.search}}</a><a href="{{url_for('rooms')}}">{{t.rooms}}</a><a href="{{url_for('occupancy_management')}}">{{t.occupancy}}</a><a href="{{url_for('occupancy_map')}}">{{t.map}}</a><a href="{{url_for('workers')}}">{{t.workers}}</a><a href="{{url_for('inspections')}}">{{t.inspections}}</a><a href="{{url_for('sector_dashboard')}}">القطاع والجولة</a><a href="{{url_for('worker_change_requests')}}">التسكين والنقل والإخلاء</a><a href="{{url_for('absence_reports_list')}}">بلاغات عدم التواجد</a><a href="{{url_for('bathroom_reports')}}">بلاغات دورات المياه</a>{% endif %}<a href="{{url_for('tickets')}}">{{t.maintenance}}</a>{% if not maintenance_only %}<a href="{{url_for('requests_list')}}">{{t.requests}}</a>{% endif %}</nav><div class="wrap"><aside class="desktop-nav"><a href="{{url_for('dashboard')}}">{{t.home}}</a><a href="{{url_for('notifications')}}">{{t.notifications}}</a>{% if not maintenance_only %}<a href="{{url_for('global_search')}}">{{t.search}}</a><a href="{{url_for('workers')}}">{{t.workers}}</a><a href="{{url_for('rooms')}}">{{t.rooms}}</a><a href="{{url_for('occupancy_management')}}">{{t.occupancy}}</a><a href="{{url_for('occupancy_map')}}">{{t.map}}</a><a href="{{url_for('inspections')}}">{{t.inspections}}</a><a href="{{url_for('sector_dashboard')}}">القطاع والجولة</a>{% endif %}{% if not maintenance_only %}<a href="{{url_for('bathroom_reports')}}">بلاغات دورات المياه</a><a href="{{url_for('requests_list')}}">{{t.requests}}</a><a href="{{url_for('worker_change_requests')}}">{{t.worker_changes}}</a>{% endif %}<a href="{{url_for('tickets')}}">{{t.maintenance}}</a>{% if u.role in ('maintenance_manager','maintenance_supervisor','housing_manager','services_manager') %}<a href="{{url_for('maintenance_dashboard')}}">{{t.maintenance_dashboard}}</a>{% endif %}{% if admin %}<a href="{{url_for('users')}}">{{t.users}}</a><a href="{{url_for('audit_logs')}}">{{t.audit}}</a>{% endif %}<a href="{{url_for('change_password')}}">{{t.change_password}}</a></aside><main>{{body|safe}}</main></div>{% else %}{{body|safe}}{% endif %}</body></html>'''
...
def worker_change_requests():
 ...
 with closing(conn()) as c:
  if is_admin(u): rows=c.execute('''SELECT q.*,ru.display_name requester,su.display_name source_sup,tu.display_name target_sup,fu.display_name final_decider FROM housing_actions q LEFT JOIN users ru ON ru.id=q.requested_by LEFT JOIN users su ON su.id=q.source_supervisor_id LEFT JOIN users tu ON tu.id=q.target_supervisor_id LEFT JOIN users fu ON fu.id=q.final_decided_by ORDER BY q.id DESC LIMIT 400''').fetchall()
  elif u['role']=='housing_supervisor': rows=c.execute('''SELECT q.*,ru.display_name requester FROM housing_actions q LEFT JOIN users ru ON ru.id=q.requested_by WHERE q.requested_by=? OR q.source_supervisor_id=? OR q.target_supervisor_id=? ORDER BY q.id DESC LIMIT 400''',(u['id'],u['id'],u['id'])).fetchall()
  else: rows=c.execute('''SELECT q.*,ru.display_name requester FROM housing_actions q LEFT JOIN users ru ON ru.id=q.requested_by WHERE q.requested_by=? ORDER BY q.id DESC LIMIT 400''',(u['id'],)).fetchall()
 return page('''<h2>التسكين والنقل والإخلاء</h2>{% if creator %}<a class="btn" href="{{url_for('new_worker_change_request')}}">طلب جديد</a>{% endif %}<div class="tbl-wrap"><table class="tbl"><tr><th>الطلب</th><th>النوع</th><th>العامل</th><th>من</th><th>إلى</th><th>الحالة</th><th>التاريخ</th><th></th></tr>{% for x in rows %}<tr><td>{{x.action_no}}</td><td>{{types.get(x.action_type,x.action_type)}}</td><td>{{x.employee_no or '-'}} - {{x.full_name or '-'}}</td><td>{{x.source_room or '-'}}</td><td>{{x.target_room or '-'}}</td><td>{{x.final_status}}</td><td>{{x.created_at}}</td><td><a href="{{url_for('worker_change_request_detail',qid=x.id)}}">فتح</a></td></tr>{% endfor %}</table></div>''','طلبات السكن',u,rows=rows,creator=can_create_housing(u),types={'add':'تسكين جديد','transfer':'نقل عامل','remove':'إخلاء عامل'})
...
def new_worker_change_request():
 ...
 if request.method=='POST':
  ...
  with closing(conn()) as c:
   ...
   if not err and typ in ('transfer','remove') and not source_sup:err='لا يوجد مشرف مسؤول عن الغرفة الحالية'
   ...
   if u['role']=='housing_supervisor':
    ...
    if decision=='approved':
     if q['action_type']=='add':
      ...
      c.execute('UPDATE workers SET room_no=?,zone=?,updated_at=? WHERE id=?',(q['target_room'],room['zone'],now(),q['worker_id']))
     else:c.execute("UPDATE workers SET archived=1,status='archived',updated_at=? WHERE id=?",(now(),q['worker_id']))
